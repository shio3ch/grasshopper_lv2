...
`src/content/blog` をファイルシステムから読んで動的に導出する（パースには Jest 側で導入する
`gray-matter` を使う。既存 spec の正規表現パースは書き換えない）。

前提: トップページは全記事一覧ではなく**最新日の記事のみ**を表示する構成（Issue #26 の
記事ファースト化）。また既存 `archive-navigation.spec.ts` が「トップの最新日見出し」
「日別ページへの遷移と記事数」「タグページの日付跨ぎ表示」を既にカバーしているため、
新設分は以下の不足分に絞る。

| ページ | 検証内容 |
|--------|----------|
| トップ `/` | 最新日の記事カードが件数どおり表示され、featured 枠が 1 件ある。カードのタイトルリンクから記事ページへ遷移できる |
| 記事 `/blog/[slug]/` | タイトル・日付（`datetime` 属性）・著者・タグが表示される |
| タグ `/tags/[tag]/` | 記事数が fixtures と一致し、全カードがそのタグを持つ |
| 404 | 存在しない URL が HTTP 404 を返す |
